...
6. [**S-Curve / Historical Progress Endpoint**](#6-s-curve--historical-progress-endpoint) ⭐ **NEW**
   - [GET /api/projects/:projectId/progress/s-curve](#61-get-apiprojectsprojectids-curve)
7. [Data Schemas (TypeScript)](#7-data-schemas-typescript)
8. [Error Handling](#8-error-handling)
9. [Backend Implementation Notes](#9-backend-implementation-notes)
...
## 6. S-Curve / Historical Progress Endpoint

> ⭐ **Section baru ini diperlukan agar grafik S-Curve di tab Summary dapat menampilkan garis Actual yang mencerminkan histori nyata di lapangan (naik-turun sesuai kecepatan kerja), bukan sekadar garis lurus diagonal.**

### Latar Belakang

Saat ini, endpoint `GET /progress/summary` hanya mengembalikan angka agregat total (`total_planned`, `total_executed`). Frontend terpaksa menarik garis lurus antara titik nol dan nilai aktual hari ini. Hal ini **tidak merepresentasikan dinamika proyek sesungguhnya**.

Dengan endpoint baru ini, backend me-return array rekaman kumulatif **per-periode (minggu atau bulan)** sehingga garis hijau (_Actual_) di grafik akan berliku-liku sesuai kenyataan lapangan.

### 6.1 GET /api/projects/:projectId/progress/s-curve

Mengambil data time-series histori progres kumulatif untuk menggambar grafik S-Curve.

**Request**

```http
GET /api/projects/PRJ-001/progress/s-curve?interval=week
Authorization: Bearer <token>
```
...
| ---------- | --------------------- | -------- | ------- | --------------------------------------------------- |
| `interval` | `"week"` \| `"month"` | ❌       | `week`  | Granularitas periode waktu untuk pengelompokan data |
...
{
  "success": true,
  "message": "S-Curve data retrieved successfully",
  "data": {
    "project_id": "PRJ-001",
    "total_planned_volume": 500,
    "interval": "week",
    "data_points": [
      {
        "period_start": "2026-03-09",
        "period_label": "09 Mar 2026",
        "actual_volume_in_period": 70,
        "actual_volume_accumulated": 70
      },
      {
        "period_start": "2026-03-16",
        "period_label": "16 Mar 2026",
        "actual_volume_in_period": 0,
        "actual_volume_accumulated": 70
      },
      {
        "period_start": "2026-03-23",
        "period_label": "23 Mar 2026",
        "actual_volume_in_period": 0,
        "actual_volume_accumulated": 70
      }
    ]
...
| Field                       | Type     | Description                                                                                     |
| --------------------------- | -------- | ----------------------------------------------------------------------------------------------- |
| `total_planned_volume`      | `number` | Total volume target proyek keseluruhan (gabungan semua termin + adendum)                        |
| `interval`                  | `string` | Interval yang digunakan: `"week"` atau `"month"`                                                |
| `data_points`               | `array`  | Array titik data per periode, **hanya dari tanggal mulai proyek sampai hari ini**               |
| `period_start`              | `string` | Tanggal awal periode, format ISO 8601: `"YYYY-MM-DD"`                                           |
| `period_label`              | `string` | Label tampilan untuk sumbu X grafik, format: `"dd MMM yyyy"` (minggu) atau `"MMM yyyy"` (bulan) |
| `actual_volume_in_period`   | `number` | Jumlah volume yang dicatat (dari `progress_records`) pada periode ini saja                      |
| `actual_volume_accumulated` | `number` | **Kumulatif** total volume aktual dari awal proyek s.d. periode ini                             |

> **Catatan Penting:**
>
> - Array `data_points` hanya memuat periode dari `project.start_date` s.d. **hari ini** (tidak termasuk periode di masa depan). Frontend akan menggabungkan ini dengan rentang waktu penuh proyek untuk menggambar garis biru (_planned_).
> - `actual_volume_in_period` dihitung dari sum `actual_volume` semua `progress_records` yang `progress_date` jatuh dalam periode tersebut.
> - `actual_volume_accumulated` adalah running total (prefix sum) dari semua `actual_volume_in_period`.
> - Jika tidak ada progress record sama sekali di suatu periode, `actual_volume_in_period = 0` dan `actual_volume_accumulated` sama dengan nilai periode sebelumnya.

**Backend Grouping Logic:**

```sql
-- Contoh query untuk interval 'week' (PostgreSQL)
SELECT
  DATE_TRUNC('week', pr.progress_date) AS period_start,
  SUM(pr.actual_volume) AS actual_volume_in_period
FROM progress_records pr
WHERE
  pr.project_id = :projectId
  AND pr.progress_date >= :project_start_date
  AND pr.progress_date <= NOW()
GROUP BY DATE_TRUNC('week', pr.progress_date)
ORDER BY period_start ASC;
```

Kemudian backend melakukan **running sum** (kumulatif) dari kolom `actual_volume_in_period` untuk mengisi `actual_volume_accumulated`.

Untuk `interval = "month"`, ganti `DATE_TRUNC('week', ...)` dengan `DATE_TRUNC('month', ...)`.

---

## 7. Data Schemas (TypeScript)

Sesuai `src/types/progress.ts` (termasuk type baru untuk S-Curve):
...
```typescript
// TYPE BARU — Untuk endpoint S-Curve
// Tambahkan ke src/types/progress.ts

// Satu titik data dalam time-series S-Curve
...
// Response body dari GET /progress/s-curve
...
| `INVALID_INTERVAL`      | 400         | Parameter `interval` bukan `week` atau `month`  |
